...
client.manager.on("playerException", async (player, data) => {
  ...
  if (player.customData.playerMessages !== "noMessage") { 
    const embed = new EmbedBuilder()
      ...
      .setTitle('Eita porra... deu merda na música! Pulando pra próxima, seu azarado!')
      .setDescription(`Música: ${data.track.info.title}\nErro: ${data.exception.cause},Node: ${player.shoukaku.node.name}\n-# Manda o desenvolvedor arrumar essa porra`);
      try {
        if (player.customData.playerMessages === "default") { // 
          ...
          console.error("Erro ao enviar mensagem de exceção do player:", err);
      }
  }
});
...
client.manager.on("playerStart", async (player, track) => {
  ...
  if (!guild.members.me.permissionsIn(channel).has(PermissionsBitField.Flags.SendMessages)) {
	...
	.setColor('#e66229')
	.setTitle(track?.title || "Tocando essa merda aqui")
	.setURL(track?.realUri || "https://youtube.com")
	.setAuthor({ name: 'Tocando essa merda aqui' })
	.setThumbnail(track?.thumbnail || "https://i.imgur.com/K9LWwgw.png")
    .setDescription(`Duração: **${convertTime(track?.length || 0, true)}** - Que lixo namoral`)
    .setTimestamp()
    .setFooter({ text: `Solicitado pelo inútil: ${track?.requester?.username}${Math.random() < 0.06 ? ' | Não quer essas mensagens? Desativa com /player-settings, seu burro' : ''}`});
