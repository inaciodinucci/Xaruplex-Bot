...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('node-manger')
    .setDescription('Apenas desenvolvedores, gerencie os nós do Lavalink.'),

  run: async ({ interaction, client, handler }) => {
    try {
      if (interaction.user.id !== process.env.OWNER_ID) return interaction.reply({ content: "Apenas desenvolvedores podem executar este comando", ephemeral: true });
      ...
      const checkRateLimit = async (nodeName) => {
        ...
        return ratelimitCheck.tracks?.length ? 'Limitado: False' : 'Limitado: True';
      };
...
\`\`\``) 
        ...
        switch (i.customId) {
          ...
          case "disconnectButton":
            ...
            if (client.cluster) {
                try {
                          ...
                          c.manager.players.forEach(async (player) => {
                              if (player.node.name === nodeName) {
                                  try {
                                      ...
                                      console.error('Erro ao mover o player:', moveError);
                                  }
                              }
                          });
                    ...
                    console.error("Erro ao desconectar o nó em algum shard:", disconnectError);
                    await interaction.followUp({ content: `Erro ao desconectar o nó ${nodeNameToDisconnect} em algum shard. Verifique os logs para mais detalhes.`, ephemeral: true });
                }
              ...
              if (nodeToDisconnect.state === 2) {
                client.manager.players.forEach(async (player) => {
                    if (player.node.name === nodeNameToDisconnect) {
                        try {
                          ...
                          console.log("erro ao mover:", error)
                        }
                    }
                });
                ...
                setTimeout(() => {
                    nodeToDisconnect.disconnect(5, 'Node discnnected by command');
                    nodeToDisconnect.ws.close();
                }, 5000);
    
    
                await interaction.followUp({ content: `Node ${nodeNameToDisconnect} desconectado. Players movidos para ${targetNode.name}`, ephemeral: true });
        
                } else {
                    await interaction.followUp({ content: `Node ${nodeNameToDisconnect} não está conectado.`, ephemeral: true });
                }
            }
            break;
          case "removeButton":
            ...
            try {
              ...
              await interaction.editReply({ embeds: [updatedEmbed], components: [row, row2], ephemeral: true });
                ...
                console.error("Erro ao remover o nó:", removeError);
              if (removeError.message.includes('Este nó não existe.')) {
                await interaction.followUp({ content: `Node ${nodeToRemove.name} não existe ou já foi removido.`, ephemeral: true });
              } else if (removeError.message.includes('Não foi possível desconectar o nó')) {
                  await interaction.followUp({ content: `Erro ao remover o nó ${nodeToRemove.name}. Ele pode estar ocupado ou em um estado ruim. Por favor, tente novamente mais tarde.`, ephemeral: true });
              } else {
                await interaction.followUp({ content: `Erro ao remover o nó ${nodeToRemove.name}: ${removeError.message}`, ephemeral: true });
              }
            ...
            case "reconnectButton":
              ...
              if (client.cluster) {
                  try {
                      ...
                      console.error("Erro ao reconectar o nó em algum shard:", reconnectError);
                      await interaction.followUp({ content: `Erro ao reconectar o nó ${nodeNameToReconnect} em algum shard. Verifique os logs para mais detalhes.`, ephemeral: true });
                  }
                ...
                if (!nodeToReconnect.connected) {
                  ...
                  await interaction.followUp({ content: `Node ${nodeNameToReconnect} reconectado.`, ephemeral: true });
          
                } else{
                  await interaction.followUp({ content: `Node ${nodeNameToReconnect} já está conectado.`, ephemeral: true });
          ...
          case "addButton":
           ...
           const nameInput = new TextInputBuilder()
            ...
            .setPlaceholder('Digite o nome do nó')
            .setRequired(true);

           const urlInput = new TextInputBuilder()
            .setCustomId('nodeUrl')
            .setLabel("URL do nó (host:port)") 
            .setStyle(TextInputStyle.Short)
           ...
           const authInput = new TextInputBuilder()
            ...
            .setLabel("Senha do nó")
            .setStyle(TextInputStyle.Short)
            .setPlaceholder('Digite a senha do nó')
            .setRequired(true);
            ...
            interaction.awaitModalSubmit({ filter: modalFilter, time: 60000 }) 
                .then(async (modalInteraction) => {
                    ...
                    try {
                        ...
                        await modalInteraction.reply({ content: `Erro ao adicionar o nó: ${error.message}`, ephemeral: true });
                    }
                })
                .catch(async(error) => {
                    console.error("Erro ao enviar o modal:", error)
                    await interaction.followUp({content: `O modal expirou ou houve um erro. O nó não foi adicionado.`, ephemeral: true}) // Handle timeout or other errors
                });
              break;
        }
      });
      ...
      console.log("erro ao executar node-manager:", error)
      interaction.editReply(`Deu erro ai caralho: ${error}`,)
    }
  },
  // devOnly: Boolean,
  //testOnly: true,
